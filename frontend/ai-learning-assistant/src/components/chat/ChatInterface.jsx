...
const ChatInterface = () => {
  ...
  useEffect(() => {
    const fetchChatHistory = async () => {
      try {
        ...
        // Backend shape: { success, data: [...messages], message }
        const messages = response?.data;

        if (Array.isArray(messages)) {
          setHistory(messages);
        } else {
          console.warn('Unexpected chat history shape:', response?.data);
          setHistory([]);
        }
      } catch (error) {
  ...
  const renderMessage = (msg, index) => {
    ...
    return (
      <div key={msg._id || index} className={`flex items-start gap-3 my-4 ${isUser ? 'justify-end' : ''}`}>
        {!isUser && (
          <div className="w-9 h-9 rounded-xl bg-linear-to-br from-emerald-400 to-teal-500 shadow-lg flex items-center justify-center shrink-0">
  ...
  return (
    <div className="flex flex-col h-[70vh] bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-2xl">
      {/* Messages Area */}
      ...
      <div className="p-5 border-t border-slate-200/60 bg-white/80">
        <form onSubmit={handleSendMessage} className="flex items-center gap-3">
          <input
            ...
            className="flex-1 h-12 px-4 border-2 border-slate-200 rounded-xl bg-slate-50/50 text-slate-800"
            disabled={loading}
          />
          <button
            type="submit"
            disabled={loading || !message.trim()}
            className="shrink-0 w-12 h-12 bg-linear-to-r from-emerald-500 to-teal-500 hover:opacity-90 rounded-xl flex items-center justify-center transition-opacity disabled:opacity-50"
          >
            <Send className="text-white" strokeWidth={2} />
          </button>
        </form>
      </div>
    </div>
